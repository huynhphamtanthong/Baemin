...
const formatData = (data, numColumns) => {
    ...
    while (number0fElementsLastRow !== numColumns && number0fElementsLastRow != 0) {
...
const Category = () => {
    ...
    const renderItem = ({item, index}) => {
        return (
            <TouchableOpacity activeOpacity={0.5} onPress={() => onHandleCategoryItemPressed(item)}>
                
                <View style={styles.categoryItem}>
                    {index == 0 && couponRotation
                    ?
                    <item.source width={40} height={40} style={styles.categoryItemRotationImage}/>
                    :
